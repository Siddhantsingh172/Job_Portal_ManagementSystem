...
class GlobalExceptionHandlerTest {
    ...
    void handleNotFoundReturns404() {
        var response = handler.handleNotFound(new ResourceNotFoundException("Job", 1L), request());
    ...
    void handleValidationReturnsFieldErrors() throws Exception {
        var response = handler.handleValidation(validationException("title", "Title is required"), request());
        assertThat(response.getStatusCode().value()).isEqualTo(400);
        assertThat(response.getBody().getFieldErrors()).containsEntry("title", "Title is required");
    }
    ...
    private HttpServletRequest request() {
        ...
        request.setRequestURI("/api/jobs/1");
        return request;
    }
